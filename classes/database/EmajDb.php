...
class EmajDb {
	...
	function isAsyncRlbkUsable($useCache = true) {
		...
		if ($this->isDblinkUsable()) {
			...
			} elseif ($this->emajVersionNum >= 40600){	// version >= 4.6.0
				$sql = "SELECT CASE
	...
	function importGroupsConfPrepare($groupsConfig, $groups) {
		...
		$sql = "SELECT rpt_severity,
				CASE rpt_msg_type
					...
					WHEN  6 THEN format('" . $data->clean($lang['strcheckconfgroups06']) . "', rpt_text_var_1, rpt_text_var_2, rpt_text_var_3)
					...
					WHEN 34 THEN format('" . $data->clean($lang['strcheckconfgroups34']) . "', rpt_text_var_1, rpt_text_var_2, rpt_text_var_3)
